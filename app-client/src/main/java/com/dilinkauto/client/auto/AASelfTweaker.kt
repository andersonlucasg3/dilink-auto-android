 ...
 *
 * Called from [com.dilinkauto.client.ClientApp.onCreate()]. On every invocation
 * it runs four phases:
 *
 *  0. **Installer phase** — ensures the package's installer is
 *     `com.android.vending`. When a sideloaded app has a different installer,
 *     this phase re-installs the APK via `pm install -r -t -i com.android.vending`,
 *     which kills the process. On the next launch the installer is correct and
 *     the remaining phases run. An anti-loop guard in SharedPreferences prevents
 *     repeated re-installs for the same app version.
 ...
 *  3. **Warmup phase** — battery whitelist + Play Store process startup
 *     so the Android Auto validator can bind to PlayGearheadService.
 *
 * ## Safety contract
 *
 ...
 * - Every failure is swallowed and logged under the `AASelfTweaker` tag. This
...
object AASelfTweaker {
    ...
    private const val TAG = "AASelfTweaker"
    ...
    private val executor = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "AASelfTweaker").apply { isDaemon = true }
    }

    /**
     * Entry point called from [com.dilinkauto.client.ClientApp.onCreate()].
     * Dispatches the check/registration to a background thread and returns immediately.
     */
    @JvmStatic
    fun ensureRegistered(context: Context) {
        ...
        executor.execute {
            try {
                ...
                FileLog.e(TAG, "ensureRegistered: unexpected failure", t)
            }
        }
    }
    ...
    private fun checkAndRegister(context: Context) {
        // 1. Root check via shared RootManager helper.
        if (!RootManager.isAvailable) {
            FileLog.w(TAG, "Root not available — skipping")
            return
        }
        ...
        //    On ROMs with aggressive Doze, the Play Store process is killed
        //    in background. When the Android Auto validator connects, it binds
        //    to PlayGearheadService in com.android.vending; if the process is
     ...
     * not the Play Store. When installed via sideload, the installer is the
     * ROM's package installer, which causes the "failed all other checks" rejection.
     *
     * This phase:
     ...
     * that failed, we skip to avoid a restart loop.
     *
    ...
    private fun ensurePlayStoreInstaller(context: Context, pkg: String): Boolean {
        ...
        val currentInstaller = try {
            ...
            FileLog.e(TAG, "Could not read installer for '$pkg'", t)
            return false
        }
    ...
    private fun ensurePhenotypeOverrides(context: Context, ourPackage: String): Boolean {
        ...
        RootManager.execFull("am kill all $GMS_PACKAGE")

        // 2. Snapshot uid/gid of the original db for later restoration.
        val (statExit, statOut, _) = RootManager.execFull("stat -c '%u %g' $PHENOTYPE_DB_PATH")
        val ownerIds = statOut.trim().split(Regex("\\s+"))
        if (statExit != 0 || ownerIds.size < 2) {
            FileLog.w(TAG, "Could not stat phenotype.db (exit=$statExit, out='$statOut') — skipping")
            return false
        }
        val uid = ownerIds[0]
        val gid = ownerIds[1]
        ...
        if (!copyDbToWorkDir(workDb)) {
            FileLog.w(TAG, "Failed to copy phenotype.db to work dir — skipping")
            return false
        }
        ...
        try {
            ...
            try {
                ...
                when {
                    hasNewSchema -> {
                        FileLog.i(TAG, "New phenotype schema detected (flag_overrides)")
                        if (isPackageWhitelistedNewSchema(db, ourPackage)) {
                            FileLog.i(TAG, "'$ourPackage' already in $FLAG_APP_WHITE_LIST — nothing to do")
                            return true
                        }
                        FileLog.i(TAG, "'$ourPackage' missing from allowlist — registering (new schema)")
                        applyNewSchemaOverrides(db, ourPackage)
                    }
                    hasLegacySchema -> {
                        FileLog.i(TAG, "Legacy phenotype schema detected (FlagOverrides)")
                        if (isPackageWhitelisted(db, ourPackage)) {
                            FileLog.i(TAG, "'$ourPackage' already in $FLAG_APP_WHITE_LIST — nothing to do")
                            return true
                        }
                        FileLog.i(TAG, "'$ourPackage' missing from allowlist — registering (legacy schema)")
                        applyFlagOverrides(db, ourPackage)
                    }
                    else -> {
                        FileLog.w(TAG, "No known overrides table (flag_overrides / FlagOverrides) " +
            ...
            val (cpExit, _, cpErr) = RootManager.execFull("cp ${workDb.absolutePath} $PHENOTYPE_DB_PATH")
            if (cpExit != 0) {
                FileLog.e(TAG, "Failed to copy patched db back: $cpErr")
                return false
            }
            FileLog.i(TAG, "Patched db copied back to $PHENOTYPE_DB_PATH")

            // 6b. Drop stale WAL/SHM at the destination so GMS cannot replay an
            //     old journal over the patched db.
            val (rmExit, _, rmErr) = RootManager.execFull("rm -f $PHENOTYPE_DB_PATH-wal $PHENOTYPE_DB_PATH-shm")
            if (rmExit != 0) {
                FileLog.w(TAG, "rm of stale -wal/-shm failed (exit=$rmExit): $rmErr")
            }

            // 7. Restore ownership and SELinux context so GMS can open the db.
            RootManager.execFull("chown $uid:$gid $PHENOTYPE_DB_PATH")
            val (rcExit, _, rcErr) = RootManager.execFull("restorecon $PHENOTYPE_DB_PATH")
            if (rcExit != 0) {
                // restorecon may not exist on some ROMs — not fatal.
                FileLog.d(TAG, "restorecon failed/absent (exit=$rcExit): $rcErr")
            }

            // 8. Restart Android Auto so it re-reads the flags.
            RootManager.execFull("am force-stop $GEARHEAD_PACKAGE")

            success = true
        } catch (t: Throwable) {
            FileLog.e(TAG, "Phenotype registration flow failed", t)
        } finally {
            deleteWorkDb(workDb)
        }

        if (success) {
            FileLog.i(TAG, "Registered '$ourPackage' in Android Auto allowlist (phenotype)")
        }
        return success
    }

    // ---- Phase 2 — Finsky forge (Play Store local validation) -------------------------
    ...
    private fun ensureFinskyRows(context: Context, pkg: String): Boolean {
        FileLog.i(TAG, "Finsky forge: checking rows for '$pkg'")

        // 1. Compute the certificate hash first — no root needed.
        val hash = getCertificateHash(context)
        if (hash == null) {
            FileLog.e(TAG, "Finsky forge: signing certificate hash unavailable — skipping")
            return false
        }
        FileLog.i(TAG, "Finsky forge: certificate hash='$hash'")

        // 2. Stop vending so it does not rewrite the dbs while we work on them.
        RootManager.execFull("am force-stop $VENDING_PACKAGE")

        // 3. Discover the owner (uid:gid) of the vending dbs at runtime —
        //    it varies by device/ROM and must never be hardcoded.
        var owner = ""
        val (lsExit, lsOut, _) = RootManager.execFull("ls -ln $LOCAL_APP_STATE_DB_PATH")
        if (lsExit == 0) {
        ...
        if (owner.isEmpty()) {
            ...
            val (_, pkgOut, _) = RootManager.execFull("cmd package list packages -U $VENDING_PACKAGE")
        ...
        if (owner.isEmpty()) {
            FileLog.w(TAG, "Finsky forge: could not determine vending owner " +
                    "(ls exit=$lsExit, out='${lsOut.trim()}') — skipping")
            return false
        }
        FileLog.i(TAG, "Finsky forge: vending data owner=$owner")
        ...
        try {
            ...
            try {
                if (!tableExists(appStateDb, "appstate")) {
                    FileLog.w(TAG, "Finsky forge: 'appstate' table missing — unsupported schema")
                    return false
                }
                if (rowExists(
                        appStateDb,
                    ...
                    FileLog.i(TAG, "Finsky forge: appstate row for '$pkg' already present")
                } else {
            ...
            try {
                if (!tableExists(libraryDb, "ownership")) {
                    FileLog.w(TAG, "Finsky forge: 'ownership' table missing — unsupported schema")
                    return false
                }
                val account = discoverFinskyAccount(libraryDb)
                FileLog.i(TAG, "Finsky forge: discovered account='$account'")
                ...
                if (rowExists(
                    ...
                    FileLog.i(TAG, "Finsky forge: ownership row with our hash already present (account='$account')")
                } else {
            ...
            if (!appStateDirty && !libraryDirty) {
                FileLog.i(TAG, "Finsky forge: all rows already in place — nothing written")
                return true
            }
            ...
            FileLog.e(TAG, "Finsky forge failed", t)
            return false
        } finally {
            deleteWorkDb(workAppState)
            deleteWorkDb(workLibrary)
     ...
     *
     * On ROMs with aggressive Doze, the Play Store process is killed in
     * background. When the validator connects to the car, it binds to
     * `com.google.android.finsky.services.PlayGearheadService` in
    ...
    private fun warmUpPlayStore(): Boolean {
        ...
        val (wlExit, wlOut, wlErr) = RootManager.execFull("dumpsys deviceidle whitelist +$VENDING_PACKAGE")
        if (wlExit == 0) {
            FileLog.i(TAG, "Play warmup: battery whitelist added ($VENDING_PACKAGE)")
        } else {
            FileLog.w(
                TAG,
        ...
        val (svcExit, svcOut, svcErr) = RootManager.execFull("am startservice -n $serviceComponent", timeoutSec = 10)
        if (svcExit == 0 || svcOut.contains("Started", ignoreCase = true)) {
            FileLog.i(TAG, "Play warmup: PlayGearheadService started via explicit component")
            return true
        }
        FileLog.d(
            TAG,
        ...
        val (actExit, actOut, actErr) = RootManager.execFull(
        ...
        if (actExit == 0 || actOut.contains("Started", ignoreCase = true)) {
            FileLog.i(TAG, "Play warmup: Play Store MainActivity started (fallback)")
            return true
        }
        FileLog.d(
            TAG,
        ...
        val (mkExit, mkOut, mkErr) = RootManager.execFull(
            "monkey -p $VENDING_PACKAGE -c android.intent.category.LAUNCHER 1",
            timeoutSec = 15
        )
        if (mkExit == 0 || mkOut.contains("Events injected: 1")) {
            FileLog.i(TAG, "Play warmup: monkey launched Play Store (last resort)")
            return true
        }
        FileLog.e(
            TAG,
        ...
        FileLog.e(TAG, "Play warmup: all methods failed — Play Store process may not be reachable")
        return false
    }
    ...
    private fun getCertificateHash(context: Context): String? {
        return try {
            ...
            val signature = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                ...
                FileLog.e(TAG, "No signing certificate found for '${context.packageName}'")
                return null
            }
            ...
            java.util.Base64.getUrlEncoder().withoutPadding().encodeToString(sha1)
        } catch (t: Throwable) {
            FileLog.e(TAG, "getCertificateHash failed", t)
            null
        }
    }
    ...
    private fun insertAppStateRow(db: SQLiteDatabase, pkg: String) {
        ...
        FileLog.i(TAG, "Finsky forge: INSERT appstate package_name='$pkg' first_download_ms=$now")
        db.execSQL(
    ...
    private fun insertOwnershipRows(db: SQLiteDatabase, pkg: String, hash: String, account: String) {
        val now = System.currentTimeMillis()
        FileLog.i(TAG, "Finsky forge: INSERT ownership doc_id='$pkg' library_id='u-tpl' account='$account' hash=''")
        db.execSQL(
        ...
        FileLog.i(TAG, "Finsky forge: INSERT ownership doc_id='$pkg' library_id='3' account='$account' hash='$hash'")
        db.execSQL(
            "INSERT OR REPLACE INTO ownership(" +
                "library_id,backend,account,doc_id,doc_type,offer_type," +
                "app_certificate_hash,shareability,purchase_time" +
    ...
    private fun pushBackFinskyDbs(
        ...
        val (exit, _, err) = RootManager.execFull(cmd)
        if (exit != 0) {
            FileLog.e(TAG, "Finsky forge: failed to copy patched dbs back (exit=$exit): ${err.trim()}")
            return false
        }
        FileLog.i(TAG, "Finsky forge: patched dbs restored")
        return true
    }
    ...
    private fun rowExists(db: SQLiteDatabase, sql: String, args: Array<String>): Boolean {
        return runCatching {
            ...
            FileLog.e(TAG, "Query failed: $sql", it)
            false
        }
    }
    ...
    private fun copyDbToWorkDir(workDb: File): Boolean {
        val (cpExit, _, cpErr) = RootManager.execFull("cp $PHENOTYPE_DB_PATH ${workDb.absolutePath}")
        if (cpExit != 0) {
            FileLog.e(TAG, "cp of phenotype.db failed: $cpErr")
            return false
        }
        // WAL/SHM companions are optional; ignore failures.
        RootManager.execFull("cp $PHENOTYPE_DB_PATH-wal ${workDb.absolutePath}-wal")
        RootManager.execFull("cp $PHENOTYPE_DB_PATH-shm ${workDb.absolutePath}-shm")
        // Root-copied files are root-owned; make them readable by our process.
        RootManager.execFull("chmod 666 ${workDb.absolutePath} ${workDb.absolutePath}-wal ${workDb.absolutePath}-shm")
        return true
    }
    ...
    private fun isPackageWhitelisted(db: SQLiteDatabase, pkg: String): Boolean {
        ...
        FileLog.i(TAG, "Current $FLAG_APP_WHITE_LIST value: '$current'")
    ...
    private fun readOverrideValue(db: SQLiteDatabase, packageName: String, name: String): String? {
        return runCatching {
            ...
            FileLog.e(TAG, "Could not read override '$name'", it)
            null
        }
    }
            ...
            db.rawQuery(
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (name == packagePrefix) {
                        FileLog.i(TAG, "Resolved config_package_id=$id for exact match '$name'")
                        return@use id
                    }
                }
                firstId?.also {
                    FileLog.i(TAG, "Resolved config_package_id=$it for prefix '$packagePrefix%'")
                }
            }
        }.getOrElse {
            FileLog.e(TAG, "Failed to resolve config_package_id for '$packagePrefix'", it)
            null
        }
    }
    ...
    private fun readNewOverrideValue(
        ...
        return runCatching {
            ...
            FileLog.e(TAG, "Could not read new-schema override '$name'", it)
            null
        }
    }
    ...
    private fun isPackageWhitelistedNewSchema(db: SQLiteDatabase, pkg: String): Boolean {
        val gmsCarId = resolveConfigPackageId(db, GMS_CAR_PACKAGE) ?: run {
            FileLog.w(TAG, "config_packages has no entry for '$GMS_CAR_PACKAGE' — cannot verify")
            return false
        }
    ...
    private fun putNewOverride(
        ...
        FileLog.i(TAG, "INSERT flag_overrides: config_package_id=$configPackageId " +
    ...
    private fun applyNewSchemaOverrides(db: SQLiteDatabase, pkg: String) {
        ...
        if (gmsCarId == null && gearheadId == null) {
            FileLog.w(TAG, "Could not resolve any config_package_id — nothing written")
            return
        }
        ...
        if (gmsCarId != null) {
            ...
            FileLog.w(TAG, "No config_package_id for '$GMS_CAR_PACKAGE' — gms.car flags skipped")
        }
        ...
        if (gearheadId != null) {
            ...
            FileLog.w(TAG, "No config_package_id for '$GEARHEAD_PACKAGE' — gearhead flags skipped")
        }

        // Best effort: the Flags table may not exist in the new schema.
        runCatching {
            db.execSQL("DELETE FROM Flags WHERE name='app_black_list'")
            FileLog.i(TAG, "DELETE FROM Flags WHERE name='app_black_list' executed")
        }.onFailure {
            FileLog.d(TAG, "DELETE FROM Flags skipped (table absent?): ${it.message}")
        }
    }
    ...
    /**
     * Applies the full set of FlagOverrides rows (legacy schema).
     * String-valued rows merge our package into the existing CSV;
     * boolean rows are unconditional.
     */
    private fun applyFlagOverrides(db: SQLiteDatabase, pkg: String) {
        runCatching { db.execSQL("DROP TRIGGER IF EXISTS aa_patched_apps") }
            .onFailure { FileLog.d(TAG, "DROP TRIGGER aa_patched_apps skipped: ${it.message}") }
        ...
        FileLog.i(TAG, "INSERT FlagOverrides: name='$FLAG_APP_WHITE_LIST' stringVal='$mergedWhiteList'")
        db.execSQL(
        ...
        FileLog.i(TAG, "INSERT FlagOverrides: name='$FLAG_BROADCAST_WHITELIST' stringVal='$mergedBroadcast'")
        db.execSQL(
            "INSERT OR REPLACE INTO FlagOverrides " +
                "(packageName,flagType,name,user,stringVal,committed) VALUES (?,0,?,?,?,0)",
        ...
        for ((owner, name) in emptyStringRows) {
            FileLog.i(TAG, "INSERT FlagOverrides: package='$owner' name='$name' stringVal=''")
            db.execSQL(
                "INSERT OR REPLACE INTO FlagOverrides " +
                    "(packageName,flagType,name,user,stringVal,committed) VALUES (?,0,?,'','',0)",
                arrayOf(owner, name)
        ...
        for ((owner, name, value) in boolRows) {
            FileLog.i(TAG, "INSERT FlagOverrides: package='$owner' name='$name' boolVal=$value")
            db.execSQL(
                "INSERT OR REPLACE INTO FlagOverrides " +
