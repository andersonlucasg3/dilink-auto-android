...
3. Правярае appVersionCode з handshake — адпраўляе `UPDATING_CAR` і аўтаматычна абнаўляе прыкладанне аўтамабіля праз dadb, калі версіі не супадаюць
...
Клас Application. Стварае каналы апавяшчэнняў (`dilinkauto_service`, `dilinkauto_update`), ініцыялізуе `UpdateManager` пры стварэнні.
...
- `installUpdate(context)`: Адкрывае сістэмны ўсталёўшчык пакетаў праз URI `FileProvider`.
- Станы: Idle, Checking, Available, Downloading, ReadyToInstall, UpToDate, Error. Прадстаўлены праз `StateFlow`.
...
- Разумны сеткавы зваротны выклік: `onLost` правярае, ці з'яўляецца страчаная сетка той, якую выкарыстоўвае злучэнне, ігнаруе незвязаныя страты (цыкліраванне мабільных даных)
...
| Accessibility Service | Ін'екцыя ўводу на фізічны дысплей (рэзервовы варыянт) |
| Notification Access | Перасылка апавяшчэнняў на аўтамабіль (з прагрэсам) |
| USB Debugging | Неабходна для трэку USB ADB аўтамабіля (Developer Options) |
