...
| appVersionCode        | int32 |  код версіі прыкладання аўтамабіля (састарэлы, для зваротнай сумяшчальнасці)
| targetFps             | int32 |  запытаны FPS аўтамабіля (напр. 60)
| даўжыня appVersionName  | int16 |  даўжыня радка versionName
| appVersionName        | UTF-8 |  назва версіі прыкладання аўтамабіля (напр. "0.16.0")
+----------------------+------+
```
...
Payload: UTF-8 package name. Аўтамабіль запытвае тэлефон выдаліць пазначаны пакет. Тэлефон апрацоўвае сістэмны дыялог выдалення і адпраўляе назад `APP_UNINSTALLED` праз канал даных пасля завяршэння.
...
Payload: `ClearNotificationMessage` — notification id + package name. Аўтамабіль закрывае адно апавяшчэнне; тэлефон ачышчае адпаведнае Android-апавяшчэнне.

### NOTIFICATION_CLEAR_ALL (0x05) — Car → Phone

Пусты payload. Аўтамабіль закрывае ўсе апавяшчэнні; тэлефон ачышчае ўсе актыўныя апавяшчэнні.

### APP_UNINSTALLED (0x06) — Phone → Car

Payload: UTF-8 package name. Тэлефон пацвярджае выдаленне прыкладання (у адказ на `APP_UNINSTALL`). Аўтамабіль выдаляе прыкладанне са сваёй сеткі launcher.

### APP_INFO_DATA (0x07) — Phone → Car

Payload: `AppInfoDataMessage` — package name, version name, version code, час усталёўкі, час абнаўлення, памер прыкладання (bytes). Тэлефон адпраўляе метаданыя прыкладання для адлюстравання ў дыялогу на баку аўтамабіля, калі карыстальнік выбірае "App Info" з кантэкстнага меню.
