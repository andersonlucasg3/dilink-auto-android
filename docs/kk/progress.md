...
Ағымдағы нұсқа: **v0.17.0** (тұрақты)
Соңғы жаңарту: 2026-05-02

## Кезеңдер

### v0.17.0 (2026-05-02)

- **Декодерді қайта іске қосу жойылды**: `MediaCodec.setOutputSurface()` `stop()`+`start()` ауыстырады. MirrorContent Compose ағашында `View.INVISIBLE` арқылы сақталады.
- **Тұрақты 30fps**: 60fps-тан 30fps-қа дейін төмендетілді. Телефон енді қызып кетпейді. CPU/GPU жүктемесі екі есе азайды.
- **Адаптивті framerate жойылды**: 15fps режимі қолданбаларды ауыстырған кезде кадрлардың төмендеуіне әкелді.
- **Құжаттама**: Барлық v0.17.0 өзгерістері 8 тілге толық аударылып құжатталды.
