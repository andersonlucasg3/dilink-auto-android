...
Жүктеме: UTF-8 package name. Виртуалды дисплейде қолданба фокус алғанда жіберіледі. Көлік мұны қолданба бақылау күйін жаңарту үшін пайдаланады.

### APP_INFO (0x17) -- Car -> Phone

Жүктеме: UTF-8 package name. Көлік телефоннан берілген пакет үшін жүйелік қолданба туралы ақпарат/баптаулар экранын ашуды сұрайды.

### APP_SHORTCUTS (0x18) -- Car -> Phone

Жүктеме: UTF-8 package name. Көлік берілген пакет үшін қолжетімді Android 7.1+ қолданба жарлықтарын сұрайды. **UI-де өшірілген** — инфрақұрылым (VD сервері сұрауы + APK XML резерві) бар, бірақ жарлықтар нақтыланғанша жасырылған (issue #57).

### APP_SHORTCUTS_LIST (0x19) -- Phone -> Car

Жүктеме: `AppShortcutsListMessage` — package name + жарлық дескрипторларының тізімі (id, shortLabel, longLabel). APP_SHORTCUTS сұрауына жауап ретінде жіберіледі.

### APP_SHORTCUT_ACTION (0x1A) -- Car -> Phone

Жүктеме: `AppShortcutActionMessage` — package name + shortcut id. Виртуалды дисплейде нақты жарлықты іске қосады.

### APP_UNINSTALL (0x1B) -- Car -> Phone

Жүктеме: UTF-8 package name. Көлік телефоннан берілген пакетті жоюды сұрайды. Телефон жүйелік жою диалогын өңдейді және аяқталғаннан кейін деректер арнасы арқылы `APP_UNINSTALLED` кері жібереді.
...
Жүктеме: `ClearNotificationMessage` — notification id + package name. Көлік бір хабарландыруды жабады; телефон сәйкес Android хабарландыруын тазартады.

### NOTIFICATION_CLEAR_ALL (0x05) — Car → Phone

Бос жүктеме. Көлік барлық хабарландыруларды жабады; телефон барлық белсенді хабарландыруларды тазартады.

### APP_UNINSTALLED (0x06) — Phone → Car

Жүктеме: UTF-8 package name. Телефон қолданбаның жойылғанын растайды (`APP_UNINSTALL` жауабы ретінде). Көлік қолданбаны іске қосу торынан жояды.

### APP_INFO_DATA (0x07) — Phone → Car

Жүктеме: `AppInfoDataMessage` — package name, version name, version code, орнату уақыты, жаңарту уақыты, қолданба өлшемі (bytes). Пайдаланушы контекстік мәзірден "Қолданба туралы ақпарат" таңдағанда, телефон көлік жағындағы диалогта көрсету үшін қолданба метадеректерін жібереді.
