...
3. Handshake-тен appVersionCode тексереді — нұсқа сәйкес келмесе `UPDATING_CAR` жібереді және көлік қолданбасын dadb арқылы авто-жаңартады
...
Application класы. Хабарландыру арналарын жасайды (`dilinkauto_service`, `dilinkauto_update`), жасау кезінде `UpdateManager` инициализациялайды.
...
- `installUpdate(context)`: `FileProvider` URI арқылы жүйелік пакет орнатушысын ашады.
- Күйлер: Idle, Checking, Available, Downloading, ReadyToInstall, UpToDate, Error. `StateFlow` арқылы ұсынылады.
...
- Ақылды желі кері шақыруы: `onLost` үзілген желінің қосылым пайдаланатын желі екенін тексереді, байланыссыз үзілістерді елемейді (мобильді деректер циклі)
...
| Accessibility Service | Физикалық дисплейге енгізуді енгізу (резерв) |
| Notification Access | Көлікке хабарландыруларды жіберу (прогрессімен) |
| USB Debugging | Көлік USB ADB трегі үшін қажет (Developer Options) |
