...
3. Порівнює appVersionName з handshake (з fallback на versionCode) — надсилає `UPDATING_CAR` та автоматично оновлює додаток автомобіля через dadb, якщо версії не збігаються
...
Клас Application. Створює канали сповіщень (`dilinkauto_service`, `dilinkauto_update`), ініціалізує `UpdateManager` та `ShizukuManager` при створенні.
...
- `installUpdate(context)`: Використовує Shizuku `pm install -r` для безшумної установки, коли Shizuku доступний; інакше відкриває системний встановник пакетів через URI `FileProvider`.
- Стани: Idle, Checking, Available, Downloading, ReadyToInstall, Installing, Installed, UpToDate, Error. Представлені через `StateFlow`.
...
- Розумний мережевий зворотний виклик: фільтрується за `TRANSPORT_WIFI` — реагує лише на зміни WiFi, ігнорує коливання мобільних даних 3G/4G
...
| Accessibility Service | Ін'єкція дотиків на віртуальний дисплей через dispatchGesture (без моніторингу подій) |
| Notification Access | Пересилання сповіщень на автомобіль (з прогресом) |
| API Shizuku | Підвищений shell-доступ для розгортання VD-сервера без ADB та безшумного самооновлення |
| QUERY_ALL_PACKAGES | Сітка лаунчера додатків |
| REQUEST_INSTALL_PACKAGES | Автооновлення додатка автомобіля через dadb |
